...
I == \A p, q \in {0, 1}: 
    /\ critReqs(p, q) :: (p#q /\ processState[p] = "critical") => /\ flag[q] = FALSE \/ turn = p \/ processState[q] = "sentRequest" 
                                                                  /\ flag[p] = TRUE
                                                                  
    /\ requestReqs(p, q) :: (p#q /\ processState[p] = "sentRequest") => flag[p] = TRUE
    
    /\ waitReqs(p, q) :: (p#q /\ processState[p] = "waiting") => flag[p] = TRUE \* /\ turn = 1 - p
...
\* Last modified Sun Jun 01 17:36:45 EDT 2025 by johnnguyen
