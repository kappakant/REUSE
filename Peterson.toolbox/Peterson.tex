...
\@x{}\moduleLeftDash\@xx{ {\MODULE} Peterson}\moduleRightDash\@xx{}%
\@pvspace{8.0pt}%
\@x{ {\EXTENDS} Integers}%
\@pvspace{8.0pt}%
\@x{ {\VARIABLES}}%
...
This is one of the three items that mutate in \ensuremath{Peterson}\mbox{'}s
 algorithm. It is what keeps track of which process\mbox{'}s turn it
 is to access the shared resource
\end{cpar}%
\end{mcom}%
\multivspace{1}%
\@x{\@s{16.4} processState ,\,}%
\begin{mcom}{284.47}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
This contains the remaining two of the three items that mutate
 in \ensuremath{Peterson\.{'}} algorithm. It is a function mapping each
 process
...
\multivspace{1}%
\@pvspace{8.0pt}%
\@x{ vars \.{\defeq} {\langle} turn ,\, processState ,\, flag {\rangle}}%
\begin{mcom}{213.49}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\@pvspace{16.0pt}%
\@x{ TypeOK \.{\defeq}}%
 \@x{\@s{16.4} \.{\land}\@s{9.74} \A\, p\@s{6.33} \.{\in} \{ 0 ,\, 1 \} \.{:}
 flag [ p ] \.{\in} \{ {\TRUE} ,\, {\FALSE} \}}%
\@x{\@s{16.4} \.{\land}\@s{9.74} turn \.{\in} \{ 0 ,\, 1 \}}%
 \@x{\@s{16.4} \.{\land}\@s{9.74} \A\, p\@s{6.33} \.{\in} \{ 0 ,\, 1 \} \.{:}
 processState [ p ] \.{\in} \{\@w{idle} ,\,\@w{sentRequest} ,\,\@w{waiting}
 ,\,\@w{critical} \}}%
\@pvspace{8.0pt}%
\@x{ TypeOk2 \.{\defeq}}%
\@x{\@s{16.4} \.{\land} turn \.{\in} \{ 0 ,\, 1 \}}%
 \@x{\@s{16.4} \.{\land} flag\@s{2.82} \.{\in} [ \{ 0 ,\, 1 \} \.{\rightarrow}
 \{ {\TRUE} ,\, {\FALSE} \} ]}%
 \@x{\@s{16.4} \.{\land} processState \.{\in} [ \{ 0 ,\, 1 \} \.{\rightarrow}
 \{\@w{idle} ,\,\@w{sentRequest} ,\,\@w{waiting} ,\,\@w{critical} \} ]}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 This is one possible initial state of \ensuremath{Peterson}\mbox{'}s
 algorithm. It is the
...
\end{lcom}%
\@x{ Init \.{\defeq}}%
 \@x{\@s{16.4} \.{\land} flag\@s{2.82} \.{=} [ i \.{\in} \{ 0 ,\, 1 \}
 \.{\mapsto} {\FALSE} ]}%
\@x{\@s{16.4} \.{\land} turn \.{\in} \{ 0 ,\, 1 \}}%
 \@x{\@s{16.4} \.{\land} processState \.{=} [ i \.{\in} \{ 0 ,\, 1 \}
 \.{\mapsto}\@w{idle} ]}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\end{lcom}%
\@x{ ProcessRequestFlag ( p ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} processState [ p ] \.{=}\@w{idle}}%
 \@x{\@s{16.4} \.{\land} flag \.{'} \.{=} [ flag {\EXCEPT} {\bang} [ p ] \.{=}
 {\TRUE} ]}%
 \@x{\@s{16.4} \.{\land} processState \.{'} \.{=} [ processState {\EXCEPT}
 {\bang} [ p ] \.{=}\@w{sentRequest} ]}%
\@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} turn {\rangle}}%
\@pvspace{8.0pt}%
\@x{ ProcessBeginWaiting ( p ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} processState [ p ] \.{=}\@w{sentRequest}}%
\@x{\@s{16.4} \.{\land} turn \.{'} \.{=} 1 \.{-} p}%
 \@x{\@s{16.4} \.{\land} processState \.{'} \.{=} [ processState {\EXCEPT}
 {\bang} [ p ] \.{=}\@w{waiting} ]}%
\@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} flag {\rangle}}%
\@pvspace{8.0pt}%
\@x{ ProcessEnterCritical ( p ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} processState [ p ] \.{=}\@w{waiting}}%
 \@x{\@s{16.4} \.{\land} ( flag [ ( 1 \.{-} p ) ] \.{=} {\FALSE} \.{\lor} turn
 \.{=} p )}%
 \@x{\@s{16.4} \.{\land} processState \.{'} \.{=} [ processState {\EXCEPT}
 {\bang} [ p ] \.{=}\@w{critical} ]}%
\@x{\@s{16.4} \.{\land} {\UNCHANGED} {\langle} flag ,\, turn {\rangle}}%
\@pvspace{8.0pt}%
\@x{ ProcessExitCritical ( p ) \.{\defeq}}%
\@x{\@s{16.4} \.{\land} processState [ p ] \.{=}\@w{critical}}%
 \@x{\@s{16.4} \.{\land} processState \.{'} \.{=} [ processState {\EXCEPT}
 {\bang} [ p ] \.{=}\@w{idle} ]}%
 \@x{\@s{16.4} \.{\land} flag \.{'} \.{=} [ flag {\EXCEPT} {\bang} [ p ] \.{=}
 {\FALSE} ]}%
...
\@x{ Next \.{\defeq}}%
\@x{\@s{16.4} \E\, p \.{\in} \{ 0 ,\, 1 \} \.{:}}%
\@x{\@s{27.72} \.{\lor} ProcessRequestFlag ( p )}%
\@x{\@s{27.72} \.{\lor} ProcessBeginWaiting ( p )}%
\@x{\@s{27.72} \.{\lor} ProcessEnterCritical ( p )}%
\@x{\@s{27.72} \.{\lor} ProcessExitCritical ( p )}%
\@pvspace{8.0pt}%
\@x{ Spec \.{\defeq} Init \.{\land} {\Box} [ Next ]_{ vars}}%
\@pvspace{8.0pt}%
 \@x{ SpecWithFairness \.{\defeq} Spec \.{\land} {\WF}_{ vars} ( Next )
 \.{\land} \A\, p \.{\in} \{ 0 ,\, 1 \} \.{:} {\WF}_{ vars} (
 ProcessRequestFlag ( p ) )}%
\@pvspace{8.0pt}%
 \@x{ MutualExclusion \.{\defeq} {\lnot} ( processState [ 0 ]
 \.{=}\@w{critical} \.{\land} processState [ 1 ] \.{=}\@w{critical} )}%
\@pvspace{8.0pt}%
...
This is a basic liveness requirement that corresponds to what is called
 ``Progress'' in the \ensuremath{Wikipedia} article. Both processes should
 eventually
 be able to enter their critical sections.
\end{cpar}%
\end{lcom}%
 \@x{ WillEventuallyEnterCritical \.{\defeq} {\Diamond} ( processState [ 0 ]
 \.{=}\@w{critical} ) \.{\land} {\Diamond} ( processState [ 1 ]
 \.{=}\@w{critical} )}%
\@pvspace{8.0pt}%
\@x{ {\THEOREM} SpecWithFairness \.{\implies} WillEventuallyEnterCritical}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 THIS INVARIANT DOES NOT \ensuremath{HOLD} AND SHOULD NOT
 \ensuremath{HOLD{\bang}} It\mbox{'}s merely
 instructive of something a reader may intuitively believe about this
 algorithm that turns out to be false.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
See the note in \ensuremath{ProcessEnterCritical}.
\end{cpar}%
\end{lcom}%
 \@x{ CanOnlyBeCriticalIfTurn \.{\defeq} \A\, p \.{\in} \{ 0 ,\, 1 \} \.{:}
 processState [ p ] \.{=}\@w{critical} \.{\implies} turn \.{=} p}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\@x{ Inv \.{\defeq} \.{\land} TypeOK}%
 \@x{\@s{34.04} \.{\land} \A\, p ,\, q \.{\in} \{ 0 ,\, 1 \} \.{:}
 processState [ p ] \.{=}\@w{critical} \.{\implies} ( flag [ q ] \.{=}
 {\FALSE} \.{\lor} turn \.{=} p )}%
\@x{\@s{34.04}}%
\@y{\@s{0}%
 \ensuremath{\.{\land} MutualExclusion
}}%
\@xx{}%
\@pvspace{8.0pt}%
 \@x{ Inv2 \.{\defeq} \A\, p ,\, q \.{:} processState [ p ]
 \.{\neq}\@w{critical}}%
 \@x{ Inv3 \.{\defeq} {\lnot} \E\, p \.{:} processState [ p ]
 \.{=}\@w{critical}}%
\@pvspace{16.0pt}%
\@x{ {\THEOREM} InitProperty \.{\defeq} Init \.{\implies} Inv}%
 \@x{\@s{16.4} {\BY} {\DEF} Init ,\, Inv ,\, ProcessRequestFlag ,\,
 ProcessBeginWaiting ,\, ProcessEnterCritical ,\, ProcessExitCritical ,\,
 TypeOK}%
\@pvspace{8.0pt}%
 \@x{ {\THEOREM} processState \.{=} [ i \.{\in} \{ 0 ,\, 1 \}
 \.{\mapsto}\@w{idle} ] \.{\implies} Inv}%
\@x{}%
\@y{\@s{0}%
 \.{??}\.{?}
}%
\@xx{}%
\@pvspace{8.0pt}%
 \@x{ {\THEOREM} InductiveProperty \.{\defeq} Inv \.{\land} Next \.{\implies}
 Inv \.{'}}%
\@x{\@s{16.4} {\BY} {\DEF} Inv ,\, Next ,\, MutualExclusion ,\, TypeOK}%
\@pvspace{8.0pt}%
 \@x{ SpecIndCheck \.{\defeq} TypeOk2 \.{\land} Inv \.{\land} {\Box} [ Next
 ]_{ vars}}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
\setboolean{shading}{false}
...
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Last modified \ensuremath{Wed} May 28
 12:21:03 \ensuremath{EDT} 2025 by \ensuremath{johnnguyen
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Wed} May 28 01:17:56
 \ensuremath{EDT} 2025 by \ensuremath{johnnguyen
}%
\end{cpar}%
\end{lcom}%
\end{document}
