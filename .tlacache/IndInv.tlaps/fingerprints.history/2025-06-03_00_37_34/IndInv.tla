...
THEOREM InductiveProperty == Inv /\ Next => Inv'
    ...
    <1>b I'
        ...
        <2>2 TAKE rm \in RMs
        <2>3 rmState'[rm] = "commit" => tmPrepared' = RMs
        
        <2>4 SUFFICES ASSUME rm \in tmPrepared
                      PROVE /\ tmState = "init"   => rmState[rm] = "prepared"
                            /\ tmState = "abort"  => rmState[rm] = "prepared" \/ rmState[rm] = "abort"
                            /\ tmState = "commit" => /\ rmState[rm] = "prepared" \/ rmState[rm] = "commit"
                                                     /\ tmPrepared = RMs 
                      BY DEF I
...
\* Last modified Tue Jun 03 00:37:33 EDT 2025 by johnnguyen
