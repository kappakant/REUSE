        ...
        <2>a CASE ProcessRequestFlag(0) \* BY DEF ProcessRequestFlag, MutualExclusion, TypeOK
            ...
            <3>2. \A p, q \in {0, 1}: (p#q /\ processState'[p] = "critical") => (flag'[q] = FALSE \/ turn' = p \/ processState'[q] = "sentRequest")
                ...
                <4>2. SUFFICES ASSUME p#q, processState'[p] = "critical"
                               ...
                               OBVIOUS
                <4>3. p#q /\ processState'[p] = "critical" BY <4>2
                <4>4. flag'[q] = FALSE \/ turn' = p \/ processState'[q] = "sentRequest" BY <2>a, <4>3 DEF ProcessRequestFlag
                
                <4>. QED BY <2>a, <4>1, <4>2 DEF Inv, TypeOK, ProcessRequestFlag
...
\* Last modified Thu May 29 14:47:30 EDT 2025 by johnnguyen
