...
THEOREM PRFInduction0 == Inv /\ Next /\ ProcessRequestFlag(0) => Inv'
    ...
    <1>2 I'
        ...
        <2>3 flag'[q] = TRUE BY <2>2, <1>1 DEF TypeOK
        <2>4 turn' = q BY <2>2, <1>1 DEF TypeOK
        <2>6 processState'[q] \in {"idle", "waiting", "critical"} BY <2>2, <1>1 DEF TypeOK
        <2>7 processState'[q] = "idle" \/ processState'[q] = "waiting" \/ processState'[q] = "critical" BY <2>6
        
        \* case for each processState value?
        <2>a CASE processState'[q] = "idle"
            <3>1 ProcessExitCritical(q) BY <2>a DEF Next, ProcessExitCritical, ProcessEnterCritical, ProcessRequestFlag, ProcessBeginWaiting
            <3>. QED
...
\* Last modified Fri May 30 10:55:09 EDT 2025 by johnnguyen
