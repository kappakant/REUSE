...
THEOREM PEnCInduction0 == Inv /\ ProcessEnterCritical(0) => Inv'
    ...
    <1>2 MutualExclusion'
        ...
        <2>6 flag[0] = FALSE \/ turn = 1 \/ processState[0] = "sentRequest" BY <2>5 DEF Inv, I
        <2>7 flag[0] = FALSE \/ turn = 1 BY <2>2, <2>6       
        <2>8 flag[0] = TRUE BY <2>2 DEF Inv, I
        <2>9 turn = 1 BY <2>7, <2>8
        
        <2>10 flag[1] = FALSE BY <2>3, <2>9
        <2>11 flag[1] = TRUE BY <2>5 DEF Inv, I
    ...
    <1>3 I'
        ...
        <2>b requestReqs(p, q)'
            ...
            <3>a CASE p = 0 /\ q = 1
                <4>1 processState' = [processState EXCEPT ![p] = "critical"] BY <3>a DEF ProcessExitCritical
                
                <4>2 processState'[p] = "critical" BY <1>1, <3>a DEF TypeOK, ProcessExitCritical
...
\* Last modified Sun Jun 01 23:57:20 EDT 2025 by johnnguyen
