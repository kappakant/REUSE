...
THEOREM PRFInduction0 == Inv /\ Next /\ ProcessRequestFlag(0) => Inv'
    ...
    <1>2 I'
        ...
        <2>1 SUFFICES ASSUME (p#q /\ processState[p] = "critical")'
                             PROVE (flag[q] = FALSE \/ turn = p \/ processState[q] = "sentRequest")'
                             BY DEF I
        <2>2 (p#q /\ processState[p] = "critical") => (flag[q] = FALSE \/ turn = p \/ processState[q] = "sentRequest") BY DEF Inv, I
        
        <2>9 (flag[q] = FALSE \/ turn = p \/ processState[q] = "sentRequest")'
        <2>10 (flag'[q] = FALSE \/ turn' = p \/ processState'[q] = "sentRequest") BY <2>9
        <2> SUFFICES ASSUME ~((flag[q] = FALSE \/ turn = p \/ processState[q] = "sentRequest")')
                     PROVE FALSE
                     OBVIOUS
        <2>. QED BY <2>1 DEF I
...
\* Last modified Fri May 30 10:30:22 EDT 2025 by johnnguyen
